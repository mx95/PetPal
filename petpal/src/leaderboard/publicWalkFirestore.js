...
export async function writePublicWalkStats({
  ...
  const payload = {
    ...
    petPhotoUrl: String(petPhotoUrl || '').trim(),
    ownerPhotoUrl: String(ownerPhotoUrl || '').trim(),
    petCategoryId: String(petCategoryId || 'dog').trim() || 'dog',
...
export async function fetchPublicLeaderboard() {
  ...
  return snap.docs.map((d) => {
    ...
    return {
      ...
      petPhotoUrl: String(x.petPhotoUrl || '').trim(),
      ownerPhotoUrl: String(x.ownerPhotoUrl || '').trim(),
      petCategoryId: String(x.petCategoryId || 'dog').trim() || 'dog',
