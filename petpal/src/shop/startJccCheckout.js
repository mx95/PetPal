...
function mapCallableError(err, t) {
  ...
  if (code === 'functions/not-found') {
    return tx(
      t,
      'shopPage.checkoutErrFunctionNotFound',
      undefined,
  ...
  if (code === 'functions/unavailable' || code === 'functions/deadline-exceeded') {
    return tx(
      t,
      'shopPage.checkoutErrFunctionUnavailable',
      undefined,
      'Cannot reach Cloud Functions. Check your network, project, and REACT_APP_FUNCTIONS_REGION (default europe-west1).'
    );
  }
  if (code === 'functions/unauthenticated') {
    return tx(t, 'shopPage.checkoutErrUnauthenticated', undefined, 'Please sign in again, then retry checkout.');
  }
  if (code === 'functions/failed-precondition' || code === 'functions/invalid-argument' || code === 'functions/permission-denied') {
    if (/unknown product/i.test(msg)) {
      return tx(
        t,
        'shopPage.checkoutErrBackendOutOfDate',
        undefined,
    ...
    return msg || tx(t, 'shopPage.checkoutErrRejected', undefined, 'Checkout was rejected. Check JCC credentials and return URL on the server.');
  }
  if (code === 'functions/internal') {
    ...
    return generic || !msg
      ? tx(
          t,
          'shopPage.checkoutErrServerFailed',
          undefined,
          'Checkout failed on the server. Inspect Cloud Function logs for createJccCheckout.'
        )
      : msg;
  }
  return msg || code || tx(t, 'shopPage.checkoutErrGeneric', undefined, 'Checkout failed.');
}

/**
 * Starts JCC hosted checkout via Cloud Function `createJccCheckout` (register.do → formUrl).
 * @param {{ sku: string, saveCard: boolean, companyId?: string, includeTracker?: boolean, includeNfc?: boolean, nfcPetIds?: string[], cartItems?: Array<{ key: string, title: string, priceCents: number, qty: number, subtitle?: string, sku?: string, saveCard?: boolean, includeTracker?: boolean, includeNfc?: boolean, nfcPetIds?: string[], recurring?: boolean }>, shippingContact?: { receiverName: string, email: string, phone: string, address: string }, t?: (key: string, params?: object) => string }} opts
 */
export async function startJccCheckout(opts) {
  ...
  if (!app) {
    throw new Error(
      tx(
        t,
        'shopPage.checkoutErrFirebaseNotConfigured',
        undefined,
  ...
  try {
    ...
    throw new Error(mapCallableError(e, t));
  }
  ...
  if (!formUrl || typeof formUrl !== 'string') {
    throw new Error(
      tx(
        t,
        'shopPage.checkoutErrMissingPaymentUrl',
        undefined,
