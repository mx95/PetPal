...
function registerXexunHttpApi(app, store) {
  const commandBases = ["/commands", "/api/tracker/commands"];

  function postAll(suffix, handler) {
    for (const base of commandBases) app.post(`${base}${suffix}`, handler);
  }

  function getAll(suffix, handler) {
    for (const base of commandBases) app.get(`${base}${suffix}`, handler);
  }
  ...
  app.get("/api", (_req, res) => {
    res.json({
      ...
      endpoints: [
        ...
        { method: "POST", path: "/commands/* OR /api/tracker/commands/*", description: "Queue device commands (both paths supported)" },
        { method: "GET", path: "/commands/pending/:imei OR /api/tracker/commands/pending/:imei", description: "Queued commands" }
      ]
    });
  });

  postAll("/queue", (req, res) => {
    const imei = imeiFrom(req);
    const command = String(req.body?.command || "").trim();
    return respondQueued(store, imei, command, res);
  });

  postAll("/ip-transfer", (req, res) => {
  ...
  postAll("/timezone/query", (req, res) => {
    const imei = imeiFrom(req);
